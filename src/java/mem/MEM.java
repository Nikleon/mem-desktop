...
public class MEM extends Application {
	...
	public void start(final Stage primaryStage) throws Exception {
		...
		scene.addEventHandler(LoginEvent.TYPE_LOGIN_ATTEMPT, evt -> {
			...
			if (valid) {
				openMainView(scene, primaryStage, creds);
			} else {
		...
		scene.addEventHandler(GuiEvent.TYPE_CLOSE_CALENDAR, evt -> {
			openMainView(scene, primaryStage, null);
	...
	private void initSettingsPane(Credentials creds) throws IOException {
		...
		FXMLLoader settingsLoader = new FXMLLoader(
				classLoader.getResource(FXML_SETTINGS_TABS));
		settingsPane = settingsLoader.<TabPane>load();
		
		// Disable tabs and fetch settings as appropriate
		SettingsController settingsController = settingsLoader
	...
	private void initModulePane(Credentials creds) throws IOException {
		...
		FXMLLoader moduleLoader = new FXMLLoader(
				classLoader.getResource(FXML_MODULE_LIST));
		modulePane = moduleLoader.<VBox>load();
		
		// Toggle modules as appropriate
		ModuleController moduleController = moduleLoader
	...
	private void initCalendarPane() throws IOException {
		FXMLLoader calendarLoader = new FXMLLoader(
				classLoader.getResource(FXML_CALENDAR_PANE));
		calendarPane = calendarLoader.<AnchorPane>load();
	}
	
	private void openMainView(Scene scene, Stage primaryStage,
			Credentials creds) {
		...
		HBox root = new HBox(settingsPane, new Separator(Orientation.VERTICAL),
				modulePane);
